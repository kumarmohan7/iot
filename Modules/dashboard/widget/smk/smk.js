...
function smk_widgetlist()
{
  ...
  var decimalsDropBoxOptions = [        // Options for the type combobox. Each item is [typeID, "description"]
	...
	var sizeoptions = [
					[14, "18"], // set size 18 to the top position to be the default value for creating new smk widgets otherwise size 40 would be always the default
					[13, "40"],
					[12, "36"],
					[11, "32"],
					[10, "28"],
	...
	addOption(widgets["smk"], "feedid",     "feedid",  _Tr("Feed"),     _Tr("Feed value"),      []);
	addOption(widgets["smk"], "colour",     "colour_picker",  _Tr("Colour"),     _Tr("Colour used for display"),      []);
	addOption(widgets["smk"], "font",     "dropbox",  _Tr("Font"),     _Tr("Font used for display"),      fontoptions);
	addOption(widgets["smk"], "fstyle",   "dropbox", _Tr("Font style"), _Tr("Font style used for display"),    fstyleoptions);
	addOption(widgets["smk"], "fweight",   "dropbox", _Tr("Font weight"), _Tr("Font weight used for display"),    fweightoptions);
	addOption(widgets["smk"], "units",      "value",   _Tr("Units"),    _Tr("Units to show"),   []);
	addOption(widgets["smk"], "decimals",   "dropbox", _Tr("Decimals"), _Tr("Decimals to show"),    decimalsDropBoxOptions);
	addOption(widgets["smk"], "size",   	"dropbox", _Tr("Size"), _Tr("Text size in px to use"),    sizeoptions);
	addOption(widgets["smk"], "unitend",  "dropbox", _Tr("Unit position"), _Tr("Where should the unit be shown"), unitEndOptions);
	addOption(widgets["smk"], "timeout",      "value",   _Tr("Timeout"),    _Tr("Timeout without feed update in seconds (empty is never)"),   []);

	return widgets;
}

function draw_smk(context,
